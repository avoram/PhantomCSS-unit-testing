...
casper.test.begin( 'Coffee machine visual tests', function ( test ) {
	...
	/*
		The test scenario
	*/
	casper.start( fs.absolute( fs.workingDirectory + '/demo/index.html' ) );
	...
	casper.then( function () {
		phantomcss.screenshot( '#parent-div', 'Parent Div with directive' );
	} );

	casper.then(function(){
		casper.sendKeys('#username','mayank');
		casper.sendKeys('#password','123');
		casper.click("#submit");
		casper.waitForSelector('#person1-div',
			function success(){
				phantomcss.screenshot('#parent-div','Parent with change view');
			},
			function timeout() {
				casper.test.fail( 'Should see controller' );
			}
		);
	});

    casper.then(function(){
        casper.click("#my-controller-link");
        casper.waitForSelector('.data-list',
            function success(){
                phantomcss.screenshot('#parent-div','Parent with change view');
            },
            function timeout() {
